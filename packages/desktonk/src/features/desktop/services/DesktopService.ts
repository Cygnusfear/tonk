...
export class DesktopService {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      console.log('[DesktopService] Initialized with', this.files.size, 'files');
    } catch (error) {
  ...
  private async setupPositionFileWatchers(): Promise<void> {
    ...
    for (const [fileId, _file] of this.files.entries()) {
      ...
      try {
        ...
        const watchId = await vfs.watchFile(positionPath, async (documentData) => {
          console.log('[DesktopService] Position file changed:', fileId, documentData);

          // Check if file was deleted (VFS might send null/empty on deletion)
          if (!documentData || !documentData.content) {
            console.log('[DesktopService] Position file deleted remotely:', fileId);
    ...
    console.log('[DesktopService] Position file watchers set up:', this.positionFileWatchers.size);
  }
